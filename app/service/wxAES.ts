...
export default class wxAES extends BaseService {

	encode(msg) {
		let { key, iv } = this.ivData();
		const appid = this.ctx.config.third.appid;
	...
	*/
	genSign(params) {
		const token = this.ctx.config.third.appid;
	...
	ivData() {
		const third = this.ctx.app.config.third;
		let key = Buffer.from(third.key + '=', 'base64');      // 解码密钥
		return { key, iv: key.slice(0, 16) }
	}
